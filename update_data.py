...
HEADERS = {
    "User-Agent": "Mozilla/5.0 (compatible; MarketDashboard/3.0; +https://github.com/)"
}
...
WAR_KEYWORDS = [
    "iran", "israel", "gaza", "middle east", "tehran", "tel aviv",
    "hezbollah", "yemen", "red sea", "hormuz",
]
GLOBAL_KEYWORDS = [
    "fed", "treasury", "yield", "oil", "brent", "wti", "gold", "dollar",
    "tariff", "inflation", "china", "economy", "macro",
]

# ── Cấu hình thị trường ────────────────────────────────────────────────────────
# Thứ tự: symbol_primary, symbol_fallbacks (danh sách các symbol thay thế để thử)
MARKET_CONFIGS: List[Dict[str, Any]] = [
    ...
    {"name": "Copper",           "symbol": "HG=F",         "decimals": 2, "category": "Metals"},
    {
        ...
        "decimals": 2, "category": "Rates",
        "transform": "tnx_pct", "suffix": "%",
    },
    ...
    {
        # VN-Index: thử nhiều symbol vì yfinance không ổn định với .VN
        "name": "VN-Index",
        "symbol": "^VNINDEX.VN",
        "symbol_fallbacks": ["^VNINDEX", "VNINDEX.VN"],
        "decimals": 2, "category": "Vietnam",
    },
...
def translate_to_vi(text: str) -> str:
    ...
    if not text or not ENABLE_TRANSLATION:
        return text
    params: Dict[str, Any] = {"q": text[:240], "langpair": "en|vi"}
    if TRANSLATE_EMAIL:
...
def fetch_rss_news(feeds: List[tuple], limit_scan: int = 20) -> List[Dict[str, Any]]:
...
def fetch_html_news(source: str, url: str, limit: int = 3) -> List[Dict[str, Any]]:
    ...
    selectors = [
        "article h3 a", "article h2 a", "h3 a", "h2 a",
        ".title a", ".news-title a", ".story a", ".post-title a",
    ]
    seen: set = set()
    items: List[Dict[str, Any]] = []
...
def _try_fetch_history(symbol: str, period: str = "30d") -> Any:
    """
    Thử tải lịch sử giá từ yfinance. Nếu thất bại trả None.
    Dùng period=30d để đảm bảo có đủ ≥2 ngày giao dịch kể cả với VN-Index.
    """
    try:
        ticker = yf.Ticker(symbol)
        hist = ticker.history(period=period, interval="1d", auto_adjust=False, actions=False)
        if hist is None or getattr(hist, "empty", True):
            return None
        closes = hist.get("Close")
        if closes is None or getattr(closes, "empty", True):
            return None
        # Cần ít nhất 2 điểm dữ liệu để tính change
        if len(closes) < 2:
            return None
        return hist
    except Exception:
...
def fetch_market(cfg: Dict[str, Any], previous: Dict[str, Any]) -> Dict[str, Any]:
    symbol = cfg["symbol"]
    fallbacks: List[str] = cfg.get("symbol_fallbacks", [])
    all_symbols = [symbol] + fallbacks

    hist = None
    used_symbol = symbol
    for sym in all_symbols:
        hist = _try_fetch_history(sym, period="30d")
        if hist is not None:
            used_symbol = sym
            break

    # Nếu tất cả đều thất bại, fallback sang dữ liệu cũ
    if hist is None:
        ...
        return {
            ...
            "note": "Lỗi: không lấy được dữ liệu từ yfinance",
        }

    try:
        closes = hist["Close"]
        last_close = _scalar(closes.iloc[-1])
        prev_close = _scalar(closes.iloc[-2])

        if last_close is None:
            raise ValueError("giá đóng cửa cuối cùng = None")
        ...
        # Ghi chú thêm nếu dùng symbol dự phòng
        sym_note = f" [{used_symbol}]" if used_symbol != symbol else ""
        return {
            "name":       cfg["name"],
            "symbol":     symbol,           # symbol gốc trong config
            "category":   cfg.get("category", "Market"),
            ...
            "decimals":   cfg.get("decimals", 2),
            "suffix":     cfg.get("suffix", ""),
            "note": f"Gần nhất: {latest_dt.astimezone(TZ).strftime('%d/%m %H:%M')}{sym_note}",
        }
...
def main() -> None:
    print(f"[{dt.datetime.now(TZ).strftime('%d/%m/%Y %H:%M')}] Bắt đầu cập nhật dữ liệu...")
    payload = build_payload()
    OUTPUT_PATH.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
    market_ok = sum(1 for m in payload["markets"] if m.get("price") is not None)
    print(
        f"✓ Đã ghi {OUTPUT_PATH.name} | "
        f"Tin: {len(payload['war_news'])}war + {len(payload['global_news'])}global + {len(payload['vn_news'])}vn | "
        f"Chỉ số: {market_ok}/{len(payload['markets'])} OK"
    )
