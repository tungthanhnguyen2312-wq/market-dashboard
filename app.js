...
const el = {
  ...
  countdown:     document.getElementById("auto-refresh-countdown"),
};
...
/** Trả true nếu note có timestamp cũ hơn 26 giờ */
function isStaleNote(note) {
  if (!note) return false;
  const m = note.match(/(\d{2})\/(\d{2})\s+(\d{2}):(\d{2})/);
  if (!m) return false;
  const [, day, month, hour, minute] = m.map(Number);
  const now = new Date();
  const dataDate = new Date(now.getFullYear(), month - 1, day, hour, minute);
  return (now - dataDate) > 26 * 3600 * 1000;
}
...
function renderNews(list, targetEl, emptyText) {
  ...
  safeRender(targetEl, list.map(item => {
    ...
    const titleOrig   = escapeHtml(fmtText(item.title));
    ...
    return `
      <a class="news-card" href="${href}" target="_blank" rel="noopener noreferrer">
        ...
        ${showOriginal ? `<div class="original-line">Gốc: ${titleOrig}</div>` : ""}
      </a>`;
  }).join(""));
}
...
function renderMarkets(list) {
  ...
  safeRender(el.marketGrid, list.map(item => {
    ...
    const stale  = isStaleNote(item.note);
    const staleClass = stale ? " stale-data" : "";
    // Cờ khi change = 0 chính xác (có thể là lỗi yfinance)
    const zeroFlag = (item.change === 0 && item.change_pct === 0 && item.price)
      ? `<span class="zero-flag" title="Dữ liệu có thể chưa cập nhật phiên mới nhất">~</span>`
      : "";
    return `
      <article class="market-card ${cls}${staleClass}">
        <div class="market-name">
          <span>${escapeHtml(fmtText(item.name))}</span>
          <span class="market-category">${escapeHtml(fmtText(item.category, ""))}</span>
        </div>
        <div class="market-price">${fmtNumber(item.price, item.decimals ?? 2)}${suffix}${zeroFlag}</div>
...
async function loadDashboardData() {
  try {
    ...
    const newsCount = (data.war_news?.length || 0)
      + (data.global_news?.length || 0)
      + (data.vn_news?.length || 0);
    ...
    console.error(err);
    const msg = `<div class="empty-state">Không tải được dữ liệu. Thử lại sau.</div>`;
    safeRender(el.warNews,    msg);
    safeRender(el.globalNews, msg);
    safeRender(el.vnNews,     msg);
    safeRender(el.marketGrid, msg);
    safeRender(el.manualNotes, msg);
...
function startAutoRefresh() {
  ...
  nextRefreshTs = Date.now() + AUTO_REFRESH_MS;
  refreshTimer = setInterval(() => {
    loadDashboardData();
  }, AUTO_REFRESH_MS);
  countdownTimer = setInterval(() => {
    const remaining = nextRefreshTs - Date.now();
    if (el.countdown) el.countdown.textContent = `Tự làm mới sau ${fmtCountdown(remaining)}`;
  }, 1000);
  if (el.countdown) el.countdown.textContent = `Tự làm mới sau ${fmtCountdown(AUTO_REFRESH_MS)}`;
}
...
async function runGemini() {
  ...
  if (!apiKey) { if (el.aiStatus) el.aiStatus.textContent = "Chưa nhập Gemini API key."; el.geminiKey?.focus(); return; }
  if (!prompt) { if (el.aiStatus) el.aiStatus.textContent = "Chưa nhập prompt."; el.aiPrompt?.focus(); return; }
