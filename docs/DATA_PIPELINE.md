...
- **Macro trên web không phải real-time:** `macro_sync.py` ghi dữ liệu thật vào bảng `macro`, giữ `macro_snapshot.csv` cho phân tích local, đồng thời sinh `data/macro_snapshot.json` và `.js` cho website. Trang chỉ đổi sau khi pipeline chạy và artifact được publish.
- **Kỳ dữ liệu tách khỏi thời gian pipeline:** từng indicator có `period`, `frequency`, `source`, `unit` và `freshness`; vì vậy CPI/GDP World Bank hiển thị kỳ năm (hiện là 2025), không mang nhãn “dữ liệu hôm nay”. Ngưỡng stale theo daily/weekly/monthly/quarterly/annual, không dùng một ngưỡng chung.
- **Khối ngoại:** kho local hiện chỉ có `foreign_room_pct` trong metadata; chưa có mua, bán hay mua bán ròng theo phiên. Web snapshot ghi `foreign_flow.status=unavailable`, tuyệt đối không suy diễn room ngoại thành dòng tiền và không điền 0.
...
- **Scanner đa khung chỉ có `1D/1W/1M`:** ngày lấy trực tiếp; tuần gom theo `W-FRI` (`open first/high max/low min/close last/volume sum`); tháng gom theo tháng lịch với ngày đại diện là phiên cuối có dữ liệu. Không có 15m/30m/1H/5H và tuyệt đối không nội suy intraday từ nến ngày.
- **`forming` khác `completed`:** 1D do pipeline sau phiên chỉ xuất hoàn chỉnh; tuần hiện tại trước lúc đóng thứ Sáu và tháng hiện tại được gắn “Đang hình thành · Chưa xác nhận · Kỳ chưa đóng”. Tuần/tháng quá khứ luôn hoàn chỉnh.
- **Confidence 0–100:** hình học tối đa 35, xu hướng 20, volume 15, hỗ trợ/kháng cự hoặc SMC 15, thanh khoản/chất lượng dữ liệu 10, RS/nền khung lớn 5. Quy đổi `80–100 = 3 sao`, `60–79 = 2`, `40–59 = 1`; dưới 40 mặc định không xuất. Mẫu lịch sử tự tính indicator tại chính bar phát hiện, không dùng snapshot tương lai để tránh look-ahead.
- **Output web riêng:** `data/candlestick_patterns.json/.js` sinh nguyên tử từ cùng một object, giới hạn mặc định 3 kết quả gần nhất mỗi mã/timeframe dù vẫn scan 90/78/36 nến. JSON strict không NaN/Infinity; JS dùng `window.CANDLESTICK_PATTERNS` cho `file://`.
- **Mã ngừng/tạm ngừng giao dịch không bị xóa:** vẫn có thể tra cứu theo `bars_ago` của chính timeframe, nhưng nếu nến cuối cũ hơn phiên quét thì nhận warning `stale_ticker_data` và bị hạ điểm chất lượng dữ liệu.
