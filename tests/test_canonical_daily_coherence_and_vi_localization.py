...
def test_value_format_node_export():
    ...
    assert "PASS_VALUE_FORMAT_NODE" in res.stdout


def test_no_hardcoded_current_session_dates_in_html_source():
    """Verify HTML sources do not contain hardcoded current-session banners or fixed counts."""
    # dashboard.html must not contain static 'Phiên 25/08/2026' or 'Phiên 28/08/2026' banner
    dashboard_html = (DASHBOARD_ROOT / "dashboard.html").read_text(encoding="utf-8")
    assert "Phiên 25/08/2026" not in dashboard_html
    assert "95 Cơ hội chiến thuật" not in dashboard_html
    assert "888/893 Khảo sát" not in dashboard_html
    assert 'id="dashboard-hero-banner"' in dashboard_html


def test_screener_controls_localized_in_html():
    ...
    assert ">Dẫn đầu</button>" in screener_html
    assert ">Động lượng</button>" in screener_html
    assert ">Thanh khoản</button>" in screener_html
    assert ">Mã sạch</button>" in screener_html
    assert "<label>Sàn<select" in screener_html
    assert "<label>Tín hiệu<select" in screener_html
    assert '<option value="current">Hiện tại</option>' in screener_html
    assert '<option value="stale">Đã cũ</option>' in screener_html


def test_release_session_contract_mismatch_fails_closed(tmp_path):
    """Verify release_session_contract fails closed on mixed sessions."""
    # Write a test directory with mismatched session
    ...
    assert any("analysis_latest.json" in m for m in mismatches)
